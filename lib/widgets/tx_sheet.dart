...
class _TxEditorState extends State<_TxEditor> {
  ...
  void _checkComplete(){
    final s = context.read<AppStore>();
    final done = s.goals.firstWhereOrNull((g)=>g.completionShown==false && g.current>=g.target);
    if(done!=null){
      done.completionShown = true;
      s.save();
      if(s.notificationsEnabled) showNotif('🎉 Цель достигнута!', '${done.title} — вся сумма собрана!');
    }
  }
